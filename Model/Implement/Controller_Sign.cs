...
{
    //public class Controller_Sign : I_Controller_Signer
    public class Controller_Sign : Controller_Sign_Test_Abstract
    {
        DAO_Post_Process dao_post_process = DAO_Post_Process.get_instance();


        public Data_Set_Employee data_set_employee { get => ((I_Signature_Employee)instance).data_set_employee; set => ((I_Signature_Employee)instance).data_set_employee = value; }
        public Data_Set_Mission_Status data_set_mission_status { get => ((I_Mission_Action)instance).data_set_mission_status; set => ((I_Mission_Action)instance).data_set_mission_status = value; }
        public Data_Set_Mission data_set_mission { get => ((I_Mission_Object)instance).data_set_mission; set => ((I_Mission_Object)instance).data_set_mission = value; }
            ...
            if (result.excute_result.isSuccesed)
            {
                ...
                if (!risk_value.Equals(""))
                {
                    ...
                    if (compare_risk_value_result.Split(',')[0].Equals("true"))
                    {
                        ...
                        //if(mission_data)
                        if (!(Risk_Arr[1].Trim() == mission_data.risk_value.Trim()))
                        {
        ...
        public override string Compare_New_Risk(string amount)
        {
            ...
            if (1000000 > compare_amount)
            {
                return ("true," + 1.ToString());
            }
            else if (1000000 < compare_amount && compare_amount <= 2000000)
            {
                return ("true," + 2.ToString());
            }
            else if (2000000 < compare_amount && compare_amount <= 3500000)
            {
                return ("true," + 3.ToString());
            }
            else if (3500000 < compare_amount && compare_amount <= 5000000)
            {
                return ("true," + 4.ToString());
            }
            else if (5000000 < compare_amount)
            {
                return ("true," + 5.ToString());
            }
        ...
        public List<string> GetTheoreticallyRequireSignTarget(Data_Set_Mission_Details mission_data, int TheoreticallyRiskValue)
        {

            List<Data_Set_Mission_require> AllTheoreticallyRequireSign = JsonConvert.DeserializeObject<List<Data_Set_Mission_require>>(LOANIT.getinstance().get_mission_require_signature(TheoreticallyRiskValue, SubSysNo.CH001));
            List<Data_Set_Mission_require> PreSendTarget = new List<Data_Set_Mission_require>();

            foreach (Data_Set_Mission_require RequireSign in AllTheoreticallyRequireSign)
            {
                PreSendTarget.Add(RequireSign);
            }
            /// mission_data.mission_type ---> 子系統代號
            /// PreSendTarget ----> 需要簽核等級的 List


            return GetSendTargetAccount(mission_data, PreSendTarget);
        }
        public List<string> GetAllRequireSignTarget(Data_Set_Mission_Details mission_data)
        {
            List<Data_Set_Mission_Collect_Signed> AllSigned = mission_data.collect_signed;
            List<Data_Set_Mission_require> AllRequireSign = mission_data.require_sign;
            List<Data_Set_Mission_require> PreSendTarget = new List<Data_Set_Mission_require>();

            foreach (Data_Set_Mission_require RequireSign in AllRequireSign)
            {
                PreSendTarget.Add(RequireSign);
            }
            ...
            ///// 判別此任務的 mission_type 再去判定 層級
            //Controller_Sign sign_controller = Controller_Sign.getInstance();

            //List<Data_Set_Mission_Employee_Level> RequireTarget = new List<Data_Set_Mission_Employee_Level>();


            //foreach (Data_Set_Mission_Employee_Level Reference_Level in sign_controller.data_set_mission_employee_level)
            //{
            //    if (Reference_Level.mission_type.Equals(mission_data.mission_type))
            //    {
            //        foreach (Data_Set_Mission_require CompareTarget in PreSendTarget)
            //        {
            //            if (Reference_Level.employee_level.Equals(CompareTarget.employee_level))
            //            {
            //                RequireTarget.Add(Reference_Level);
            //            }
            //        }
            //    }
            //}
            //List<string> Account_List = new List<string>();
            //foreach (Data_Set_Mission_Employee_Level LevelChart in RequireTarget)
            //{
            //    foreach (Data_Set_Employee_Account AccountChart in LevelChart.account_list)
            //    {
            //        Account_List.Add(AccountChart.account);
            //    }
            //}
            //return Account_List;
            return GetSendTargetAccount(mission_data, PreSendTarget);
        }
        public List<string> GetAllNotSignedTarget(Data_Set_Mission_Details mission_data)
        {
            List<Data_Set_Mission_Collect_Signed> AllSigned = mission_data.collect_signed;
            List<Data_Set_Mission_require> AllRequireSign = mission_data.require_sign;
            List<Data_Set_Mission_require> PreSendTarget = new List<Data_Set_Mission_require>();
            bool SignedSwitch = false;

            foreach (Data_Set_Mission_require RequireSign in AllRequireSign)
            {
                Data_Set_Mission_require temp_RequireSign = new Data_Set_Mission_require();
                foreach (Data_Set_Mission_Collect_Signed CollectSign in AllSigned)

                {
                    temp_RequireSign = RequireSign;
                    if (RequireSign.employee_level.Equals(CollectSign.employee_level) && (CollectSign.sign_status.ToUpper().Equals("PASS") || CollectSign.sign_status.ToUpper().Equals("DONE")))
                    {
                        SignedSwitch = true;
                    }
                }
                if (SignedSwitch == false)
                {
                    PreSendTarget.Add(temp_RequireSign);
                }
                SignedSwitch = false;
            }
        ...
        public List<string> GetNextSignedTarget(Data_Set_Mission_Details mission_data)
        {
            List<Data_Set_Mission_Collect_Signed> AllSigned = mission_data.collect_signed;
            List<Data_Set_Mission_require> AllRequireSign = mission_data.require_sign;
            List<Data_Set_Mission_require> PreSendTarget = new List<Data_Set_Mission_require>();
            bool SignedSwitch = false;
            AllRequireSign.Sort((x, y) => -x.employee_level.CompareTo(y.employee_level)); //由大到小排序好 因為要根據逐級規則，尋找目前換誰
            foreach (Data_Set_Mission_require RequireSign in AllRequireSign)
            {
                Data_Set_Mission_require temp_RequireSign = new Data_Set_Mission_require();
                foreach (Data_Set_Mission_Collect_Signed CollectSign in AllSigned)
            ...
            //    if (Reference_Level.mission_type.Equals(mission_data.mission_type))
            //    {
            //        if (Reference_Level.employee_level.Equals(PreSendTarget[0].employee_level)) /// 在這邊鎖死 只要找第一個；在上面已經將排序鎖好找出下一個簽層
            //        {
            //            RequireTarget.Add(Reference_Level);
            //        }
            //    }
        ...
        public List<string> GetSendTargetAccount(Data_Set_Mission_Details mission_data, List<Data_Set_Mission_require> PreSendTarget)
        {
            List<Data_Set_Mission_Employee_Level> RequireTarget = new List<Data_Set_Mission_Employee_Level>();
        ...
        {
            return ((I_Signature_Employee)instance).set_employee_mission_information(data_set_employee, value);
        }

        public Data_Set_Employee set_employee_signed_information(Data_Set_Employee data_set_employee, string value)
        {
            return ((I_Signature_Employee)instance).set_employee_signed_information(data_set_employee, value);
        }

        public Data_Set_Employee set_employee_prepare_sign_information(Data_Set_Employee data_set_employee, string value)
        {
            return ((I_Signature_Employee)instance).set_employee_prepare_sign_information(data_set_employee, value);
        }

        public Data_Set_Employee set_employee_agent_information(Data_Set_Employee data_set_employee, string value)
        {
            return ((I_Signature_Employee)instance).set_employee_agent_information(data_set_employee, value);
        }

        public string update_mission_data(string m_id)
        {
            return ((I_Mission_Action)instance).update_mission_data(m_id);
        }

        public string register_mission_status_summary(Data_Set_Mission_Status data_set_mission_status)
        {
            return ((I_Mission_Action)instance).register_mission_status_summary(data_set_mission_status);
        }
        ...
        public Data_Set_Mission_Collect_Signed get_Previous_SignPassData(Data_Set_Mission_Details mission_data)
        {
            List<Data_Set_Mission_Collect_Signed> collect_sign = mission_data.collect_signed;
            collect_sign.Sort((x, y) => x.employee_level.CompareTo(y.employee_level));
            foreach (Data_Set_Mission_Collect_Signed item in collect_sign)
            {
                if (item.sign_status.ToUpper().Equals("PASS") || item.sign_status.ToUpper().Equals("DONE"))
                {
                    return item;
                }
            }
        ...
        public Data_Set_Mission_Collect_Signed get_Previous_SignFailData(Data_Set_Mission_Details mission_data)
        {
            List<Data_Set_Mission_Collect_Signed> collect_sign = mission_data.collect_signed;
            collect_sign.Sort((x, y) => x.employee_level.CompareTo(y.employee_level));
            foreach (Data_Set_Mission_Collect_Signed item in collect_sign)
            {
                if (item.sign_status.ToUpper().Equals("FAIL"))
                {
                    return item;
                }
            }
        ...
        /// 封裝以及 SingleTon
        /// </summary>
        private Controller_Sign()
        {
            Current_Token = new Date_Set_Login();
            Load_Employee_Level_Reference();
        }

        public static Controller_Sign instance = new Controller_Sign();
        public static Controller_Sign getInstance()
        {
            return instance;
        }
        #endregion
